                    ...
                    <div className="relative z-10 grid md:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="flex flex-col items-center">
                            <div className="w-full aspect-[2/1] bg-gradient-to-br from-primary-500/20 to-secondary-500/20 rounded-2xl border border-white/10 flex flex-col items-center justify-center relative overflow-hidden shadow-glow">
                                <div className="text-sm text-primary-200 uppercase tracking-wider mb-2">{t('lotteryApi')}</div>
                        ...
                        <div>
                            <div className="flex items-center gap-4 mb-6">
                                <span className={`px-4 py-1.5 rounded-full text-sm font-bold ${ticket.status === 'Available' ? 'bg-success/20 text-success' :
                                    ...
                                    {ticket.status === 'Available' ? t('statusAvailable') : ticket.status === 'Reserved' ? t('statusReserved') : t('statusSold')}
                                </span>
                                <span className="text-gray-400 text-sm">{t('set')} {ticket.set || 1}</span>
                            </div>
                            ...
                            <p className="text-gray-400 mb-8">
                                {t('drawDate')} {ticket.round?.drawDate ? new Date(ticket.round.drawDate).toLocaleDateString(locale === 'th' ? 'th-TH' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : '-'}
                            </p>

                            <div className="flex items-end gap-2 mb-8">
                                <span className="text-5xl font-bold text-primary-400">฿ {ticket.price}</span>
                                <span className="text-gray-400 mb-2">{t('baht')}</span>
                            </div>
                            ...
                            <div className="flex gap-4">
                                {isInCart ? (
                                    ...
                                    >
                                        {isAvailable ? t('addToCart') : t('cannotBuy')}
                                    </Button>
                                )}
                            </div>
                        </div>
